...
contains a random nonce (ClientHellorandom), its preferences for
...
Once the client received the ServerKeyExchange, it can also compute
...
ServerHello will indicate the mismatch and the client will need to
restart the handshake with an appropriate ClientHello/ClientKeyExchange
...
processed and transmitted as specified by the current active session state.
       ...
       struct {
           ...
           select (HandshakeType) {
               ...
               case server_key_exchange: ServerParameters;
...
security parameters in an existing connection. Finally, the client MUST
...
for one or more key establishment methods.
...
When one of the ClientKeyExchangeOffers is a Diffie-Hellman key, the client
SHALL encode it using ClientDiffieHellmanParams.
...
### Server Parameters Message

When this message will be sent:

> This message will be sent immediately after the ServerHello message if
the client has not provided a ClientKeyExchange message which is compatible
with the selected cipher suite.


Meaning of this message:

> This message conveys the server's cryptographic parameters to the
client.  When combined with ServerHello, ServerParameters is intended
to give the client enough information to construct an acceptable
ClientKeyExchange message. For instance, when DHE with explicit
(i.e., un-named) groups is used, it will contain the server's
...
[[OPEN ISSUE: This message isn't needed at all for ECDHE because
the extensions indicate the selected group. It is also not needed
for groups in {{I-D.gillmor-tls-negotiated-dl-dhe}}. If we mandate
that for TLS 1.3, we can probably dispense with this message, which
would be a good thing because it is clunky.]]

[[OPEN ISSUE: There is no way for the server to provide his public
key, just the group parameters. This means that we don't yet have
a way for the server to signal a public key for encrypting SNI.
Per the discussion in the interim, we need to add that. The natural
way is by supplying a ServerKeyExchange, since that would survive
elimination of this message as above. However, it would also
mean that we needed a ServerHelloDone to allow that message
to be optional.]]
...
Structure of this message:
       ...
       struct {
           ...
           select (KeyExchangeAlgorithm) {
               case dhe:
                   ServerDiffieHellmanParams params;
               /* may be extended, e.g., for ECDH -- see [RFC4492] */
           };
       } ServerKeyExchange;

client_params_index
: A 0-based index into the client's ClientKeyExchange representing which
of the ClientKeyExchangeOffers the server is providing a share for.
[OPEN ISSUE: We might be able to dispense with this if the ciphersuite
and other extensions uniquely specified the client's parameters. This
is true with ECDHE but is not true for DHE. If
{{I-D.gillmor-tls-negotiated-dl-dhe}} were mandatory, it would be, however.]
...
[TODO: Server Certificate Verify; Same as the client.]
